...
<div class="container-fluid p-0">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold fs-4 m-0 text-white">{{ isset($blog) ? 'Edit' : 'Create' }} <span class="text-red italic">Post</span></h2>
        ...
        <form action="{{ isset($blog) ? route('admin.blogs.update', $blog->id) : route('admin.blogs.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                <div class="col-md-8">
                    ...
                    <input type="text" name="title" class="form-control bg-dark border-secondary text-white" value="{{ old('title', $blog->title ?? '') }}" required>
                </div>
                
                <div class="col-md-4">
                    <label class="form-label text-muted small text-uppercase fw-bold">Slug</label>
                    <input type="text" name="slug" class="form-control bg-dark border-secondary text-white" value="{{ old('slug', $blog->slug ?? '') }}" required>
                </div>

                <div class="col-md-6">
                    <label class="form-label text-muted small text-uppercase fw-bold">Author</label>
                    <input type="text" name="author" class="form-control bg-dark border-secondary text-white" value="{{ old('author', $blog->author ?? 'Admin') }}" required>
                </div>
                ...
                <div class="col-12">
                    ...
                    <textarea name="content" class="form-control bg-dark border-secondary text-white" rows="10" required>{{ old('content', $blog->content ?? '') }}</textarea>
                </div>
                ...
                <div class="col-12">
                    <div class="form-check form-switch">
                        ...
                        <label class="form-check-label text-white" for="is_published">Publish immediately</label>
                    </div>
                </div>
            </div>
