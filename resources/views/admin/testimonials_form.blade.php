...
<div class="container-fluid p-0">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold fs-4 m-0 text-white">{{ isset($testimonial) ? 'Edit' : 'Add' }} <span class="text-red italic">Testimonial</span></h2>
        ...
        <form action="{{ isset($testimonial) ? route('admin.testimonials.update', $testimonial->id) : route('admin.testimonials.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" class="form-control bg-dark border-secondary text-white" value="{{ old('name', $testimonial->name ?? '') }}" required>
                </div>
                
                <div class="col-md-6">
                    <label class="form-label text-muted small text-uppercase fw-bold">Role / Title</label>
                    <input type="text" name="role" class="form-control bg-dark border-secondary text-white" value="{{ old('role', $testimonial->role ?? '') }}" placeholder="e.g. Member, CEO">
                </div>
                ...
                <div class="col-12">
                    ...
                    <textarea name="message" class="form-control bg-dark border-secondary text-white" rows="4" required>{{ old('message', $testimonial->message ?? '') }}</textarea>
                </div>
